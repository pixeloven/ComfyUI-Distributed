...
export class ApiClient {
  ...
  async getWorkerLog(
    ...
    return this.request<{ log: string }>(
      `/distributed/worker_log/${workerId}?lines=${lines}`
    )
  }
  ...
  async checkStatus(
    url: string,
    timeout: number = TIMEOUTS.STATUS_CHECK
  ...
  async checkMultipleStatuses(
    urls: string[]
  ): Promise<PromiseSettledResult<StatusResponse>[]> {
