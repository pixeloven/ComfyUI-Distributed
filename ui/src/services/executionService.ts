...
export class ExecutionService {
  ...
  private setupInterceptor() {
    ...
    if (!comfyAPI) {
      console.error('ComfyUI API not available - cannot set up execution interceptor');
      return;
    }
   ...
   */
  private findNodesByClass(apiPrompt: any, className: string): DistributedNode[] {
  ...
  private async prepareApiPromptForParticipant(
    ...
    for (const upscaleNode of upscaleNodes) {
      ...
      const uniqueJobId = options.job_id_map.get(upscaleNode.id) || upscaleNode.id;
  ...
  private async executeJobs(jobs: JobExecution[]): Promise<any> {
    ...
    const promises = jobs.map(job => {
      if (job.type === 'master') {
        return this.originalQueuePrompt(0, job.promptWrapper).then((result: any) => {
   ...
   */
  private async dispatchToWorker(worker: any, prompt: any, workflow: any): Promise<void> {
    ...
    try {
      ...
      console.error(`Failed to connect to worker ${worker.name} at ${workerUrl}:`, error);
    }
  }
  ...
  private async performPreflightCheck(workers: any[]): Promise<any[]> {
    ...
    console.log(`Performing pre-flight health check on ${workers.length} workers...`);
